...
const N8N_FOUNDER_REVIEW_WEBHOOK = "http://168.144.71.48:5678/webhook-test/13246890-0caf-43e4-adac-1ac25f4add1b";

async function handleFounderDecision(draft: any, decision: string, feedback: string = "") {
  try {
    const response = await fetch(N8N_FOUNDER_REVIEW_WEBHOOK, {
      method: "POST",
      headers: {
...
export default function FoundersTextPage() {
  const { data, error, isLoading, mutate } = useSWR<ContentDraft[]>(
    '/api/content-drafts?status=pending_founders',
    fetcher,
  ...
  const [approvedSessionCount, setApprovedSessionCount] = usePersistentSessionCount('founder_text_approved');
  const [rejectedSessionCount, setRejectedSessionCount] = usePersistentSessionCount('founder_text_rejected');
  const [editedSessionCount, setEditedSessionCount] = usePersistentSessionCount('founder_text_edited');
  ...
  const handleApprove = async (draft: any) => {
    ...
    mutate();
    handleFounderDecision(draft, "approve");
  };
  ...
  const handleReject = async (draft: any, feedback: string) => {
    ...
    mutate();
    handleFounderDecision(draft, "edit", feedback);
  };
  ...
  const handleHardReject = async (draft: any) => {
    ...
    mutate();
    handleFounderDecision(draft, "reject");
  };
  ...
  rawDrafts.forEach(d => {
    ...
    if (sX === 'pending_founders' && d.finalDraft?.x) {
    ...
    if (sLI === 'pending_founders' && d.finalDraft?.linkedin) {
    ...
    if (sInsta === 'pending_founders' && d.finalDraft?.instagram) {
              ...
              return (
                <div key={job.jobId} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 20 }}>
                    ...
                    <div>
                      {jobLI ? (
                        <ContentCard key={jobLI.virtualId} draft={jobLI} actions={[]} mode="founders"
                          onApprove={() => handleApprove(jobLI)}
                          onComment={feedback => handleReject(jobLI, feedback)}
                          onDisapprove={() => handleHardReject(jobLI)}
                        />
                      ) : (
                    ...
                    <div>
                      {jobX ? (
                        <ContentCard key={jobX.virtualId} draft={jobX} actions={[]} mode="founders"
                          onApprove={() => handleApprove(jobX)}
                          onComment={feedback => handleReject(jobX, feedback)}
                          onDisapprove={() => handleHardReject(jobX)}
                        />
                      ) : (
                        <div style={{ padding: '24px 0', textAlign: 'center', color: '#ccc', fontSize: 12.5, border: '1px dashed #eaeaea', borderRadius: 16 }}>
                          Not generated or already reviewed
                    ...
                    <div>
                      {jobInsta ? (
                        <ContentCard key={jobInsta.virtualId} draft={jobInsta} actions={[]} mode="founders"
                          onApprove={() => handleApprove(jobInsta)}
                          onComment={feedback => handleReject(jobInsta, feedback)}
                          onDisapprove={() => handleHardReject(jobInsta)}
                        />
                      ) : (
                        <div style={{ padding: '24px 0', textAlign: 'center', color: '#ccc', fontSize: 12.5, border: '1px dashed #eaeaea', borderRadius: 16 }}>
                          Not generated or already reviewed
                ...
                <div style={{
                  ...
                  {cfg.drafts.map(draft => (
                    <div key={draft.virtualId} style={{ display: 'flex', flexDirection: 'column' }}>
                      ...
                      <ContentCard draft={draft} actions={[]} mode="founders"
                        onApprove={() => handleApprove(draft)}
                        onComment={feedback => handleReject(draft, feedback)}
                        onDisapprove={() => handleHardReject(draft)}
                      />
                    </div>
                  ))}
                </div>
